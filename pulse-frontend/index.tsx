...
// Feed Section (Updated with Confirm IV)
const FeedSection: React.FC<{ title: string; items: FeedItem[] }> = ({ title, items }) => {
    const [confirmingIV, setConfirmingIV] = useState<number | null>(null);

    const handleConfirmIV = async (item: FeedItem) => {
        setConfirmingIV(item.id);
        // TODO: Call Gemini Thinking to recalculate IV
        // This would call a backend endpoint with deepThinking=true
        setTimeout(() => setConfirmingIV(null), 2000); // Mock for now
    };
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-1">
                {items.map((item) => (
                    <div key={item.id} className="p-2 border-l-2 border-[#FFC038]/30 hover:border-[#FFC038] bg-[#FFC038]/5 hover:bg-[#FFC038]/10 transition-all group font-mono">
                        ...
                        {item.iv && (
                            <div className="mt-1.5 flex items-center justify-between gap-2 text-[9px] font-mono tracking-wide border-t border-[#FFC038]/10 pt-1">
                                ...
                                <button
                                    onClick={() => handleConfirmIV(item)}
                                    disabled={confirmingIV === item.id}
                                    className="flex items-center gap-1 px-1.5 py-0.5 rounded border border-[#FFC038]/30 hover:border-[#FFC038] hover:bg-[#FFC038]/10 transition-all disabled:opacity-50"
                                    title="Confirm IV with advanced reasoning"
                                >
                                    {confirmingIV === item.id ? (
                                        <Loader className="w-3 h-3 text-[#FFC038] animate-spin" />
                                    ) : (
                                        <Brain className="w-3 h-3 text-[#FFC038]" />
                                    )}
                                    <span className="text-[8px] font-bold uppercase tracking-wider text-[#FFC038]">Confirm</span>
                                </button>
                            </div>
                        )}
                    </div>
...
const SuggestionChips = ({ onSelect }: { onSelect: (prompt: string) => void }) => {
    const suggestions = [
        { icon: <Newspaper className="w-3 h-3" />, text: "Check the tape", prompt: "Check the tape" },
        { icon: <FileText className="w-3 h-3" />, text: "Run the NTN report", prompt: "Run the NTN report" },
        { icon: <Activity className="w-3 h-3" />, text: "Run psych eval", prompt: "Run psych eval" },
        { icon: <Target className="w-3 h-3" />, text: "What's the lean?", prompt: "What's the market lean right now?" },
        { icon: <Brain className="w-3 h-3" />, text: "Analyze IV trends", prompt: "Analyze the IV trends from recent news" },
    ];

    return (
        <div className="p-4 space-y-2">
            <div className="text-[10px] text-[#FFC038]/50 uppercase tracking-widest font-bold mb-3">Suggested Prompts</div>
            <div className="flex flex-wrap gap-2">
                {suggestions.map((suggestion, idx) => (
                    <button
                        key={idx}
                        onClick={() => onSelect(suggestion.prompt)}
                        className="flex items-center gap-2 px-3 py-2 bg-[#140a00] border border-[#FFC038]/20 hover:border-[#FFC038] hover:bg-[#FFC038]/5 rounded-lg transition-all group"
                    >
                        <span className="text-[#FFC038]/60 group-hover:text-[#FFC038]">{suggestion.icon}</span>
                        <span className="text-[11px] text-[#FFC038]/70 group-hover:text-[#FFC038] font-['Roboto']">{suggestion.text}</span>
                    </button>
                ))}
            </div>
        </div>
...
const ChatInterface = ({
    ...
    const handleSendAgent = async (deepThinking: boolean = false) => {
        ...
        const agentSettings = {
            ...
            deepThinking // Pass deep thinking mode
        };
        ...
        <div className="flex flex-col h-full bg-[#0a0a00] relative">
            <div className="flex-1 overflow-y-auto p-4 space-y-6">
                {!activeThread ? (
                    <div className="flex flex-col h-full">
                        <div className="flex flex-col items-center justify-center flex-1 text-[#FFC038]/30">
                            <GeminiIcon className="w-16 h-16 mb-4 opacity-50" />
                            <p className="font-mono text-sm mb-6">Uplink Ready. Initialize Chat.</p>
                        </div>
                        <SuggestionChips onSelect={(prompt) => {
                            setInput(prompt);
            ...
            <div className="p-4 bg-black/80 backdrop-blur border-t border-[#FFC038]/20 relative">
                ...
                <div className={cn(
                    ...
                    <button
                        onClick={() => handleSendAgent(true)}
                        disabled={thinking || !input.trim()}
                        className={cn(
                            "mb-2 transition-all duration-200 flex items-center gap-1 px-2 py-1 rounded group",
                            input.trim() && !thinking ? "text-[#FFC038] hover:bg-[#FFC038]/10 border border-[#FFC038]/30" : "text-[#FFC038]/30 cursor-not-allowed border border-transparent"
                        )}
                        title="Think Harder - Uses advanced reasoning model"
                    >
                        <Brain className="w-4 h-4" />
                        <span className="text-[9px] font-mono font-bold uppercase tracking-wider">Think Harder</span>
                    </button>

                    <button
                        onClick={() => handleSendAgent(false)}
                        disabled={thinking || !input.trim()}
                        className={cn(
        ...
        <Modal isOpen={isOpen} onClose={onClose} className="w-full max-w-2xl">
            <div className="p-6">
                ...
                <div className="space-y-8">
                    ...
                    {/* API Keys - REMOVED as part of Backend Integration */}
...
const AppContent = () => {
    ...
    useEffect(() => {
        const fetchFeed = async () => {
            const isPro = user.tier === 'pulse_pro';
            const usingMock = !isPro;
            let newRawItems = [];
            ...
            if (usingMock) {
                ...
                try {
                    // Fetch from Pulse Newswire Backend
                    const res = await fetch('https://pulse-pricedin.workers.dev/api/newswire');

                    if (!res.ok) throw new Error("Uplink Failure");

                    const data = await res.json();
                    // Assuming the worker returns { news: [...] } or just [...]
                    // Adjust based on actual worker response structure. 
                    // For now, assuming it matches the structure we send or similar.
                    const items = data.news || data.data || data;

                    if (Array.isArray(items)) {
                        newRawItems = items.map((item: any) => ({
                            id: item.id || Date.now(),
                            created_at: item.timestamp ? new Date(item.timestamp * 1000).toISOString() : new Date().toISOString(),
                            text: item.headline || item.text,
                            source: item.source || 'Pulse Wire'
                        }));
                    }
                } catch (e) {
                    console.warn("Feed Uplink Failed, switching to simulation.", e);
                    const mockItem = MOCK_WIRE_DATA[Math.floor(Math.random() * MOCK_WIRE_DATA.length)];
                    newRawItems = [{
                        id: Date.now(),
                        created_at: new Date().toISOString(),
                        text: mockItem.text + " [OFFLINE]",
                        source: mockItem.source
                    }];
                }
            }
