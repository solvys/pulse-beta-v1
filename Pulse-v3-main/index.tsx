...
import { 
  ...
  Link as LinkIcon, Square, Trash2, Play, Calendar
} from 'lucide-react';
...
const FeedSection: React.FC<{ title: string; items: FeedItem[] }> = ({ title, items }) => (
  <div className="relative mb-4 h-full flex flex-col">
    ...
    <div className="flex-1 overflow-y-auto p-2 space-y-1">
      {items.map((item) => {
        const sentiment = item.iv ? (item.iv.value >= 0 ? "BULLISH" : "BEARISH") : "NEUTRAL";
        const sentimentColor = sentiment === "BULLISH" ? "text-emerald-500" : (sentiment === "BEARISH" ? "text-red-500" : "text-[#FFC038]/50");
        
        return (
            <div key={item.id} className="p-3 border-l-2 border-[#FFC038]/30 hover:border-[#FFC038] bg-[#FFC038]/5 hover:bg-[#FFC038]/10 transition-all group font-mono flex flex-col gap-2">
              <div className="flex justify-between items-baseline">
                <span className="text-[9px] text-[#FFC038]/50">{item.time}</span>
                {item.symbol && <span className="text-[10px] font-bold text-[#FFC038] tracking-wider">{item.symbol}</span>}
              </div>
              <p className="text-[11px] leading-relaxed text-[#FFC038]/90 group-hover:text-[#FFC038]">
                {item.source && <span className="text-[#FFC038]/60 mr-1 uppercase text-[9px]">[{item.source}]</span>}
                {item.text}
              </p>
              {item.iv && (
                  <div className="flex items-center justify-between text-[9px] font-mono tracking-wide border-t border-[#FFC038]/10 pt-2 mt-1">
                      <div className="flex items-center gap-2">
                        <span className={cn("px-1.5 py-0.5 rounded bg-black border border-current font-bold", item.iv.type === 'cyclical' ? "text-emerald-500/80 border-emerald-500/30" : "text-amber-500/80 border-amber-500/30")}>
                            {item.iv.type.toUpperCase()}
                        </span>
                        <span className={cn("font-bold", sentimentColor)}>{sentiment}</span>
                      </div>
                      <div className="flex items-center gap-1">
                          <span className="text-[#FFC038] opacity-50">IV:</span>
                          <span className={cn("font-bold", item.iv.value >= 0 ? "text-emerald-500" : "text-red-500")}>
                              {item.iv.value >= 0 ? '▲' : '▼'} {Math.abs(item.iv.value).toFixed(1)}pts
                          </span>
                      </div>
                  </div>
              )}
                ...
                {groupOrder.map(group => {
                    ...
                    return (
                        <div key={group} className="space-y-1">
                            <h4 className="text-[9px] font-bold text-[#FFC038]/40 uppercase tracking-widest px-1 mb-1">{group}</h4>
                            {groupThreads.map(t => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className="flex items-center gap-2 text-[9px] font-mono mt-1 opacity-70 group-hover:opacity-100">
                                            <div className="flex items-center gap-1 bg-[#140a00] px-1 rounded border border-[#FFC038]/10">
                                            ...
                                            <div className="flex items-center gap-1 bg-[#140a00] px-1 rounded border border-[#FFC038]/10">
                                                ...
                                                <span className="text-[#FFC038]/60">{resonance === 'tilt' ? 'TILT' : 'STABLE'}</span>
                                            </div>
                                        </div>
                                    </button>
                                );
        ...
        <Modal isOpen={isOpen} onClose={onClose} className="w-full max-w-2xl">
            <div className="p-6">
                ...
                <div className="space-y-8">
                    ...
                    {/* Active Markets (Read-only) */}
                    <section>
                        <h3 className="text-xs font-bold text-[#FFC038]/50 uppercase border-b border-[#FFC038]/20 pb-2 mb-4">Active Markets</h3>
                        <div className="flex flex-wrap gap-2 p-3 bg-[#140a00] rounded border border-[#FFC038]/10">
                            {['/MES', '/MNQ', '/MGC', '/SI'].map(ticker => (
                                <span key={ticker} className="px-2 py-1 bg-black rounded border border-[#FFC038]/30 text-[#FFC038] text-xs font-bold font-mono">
                                    {ticker}
                                </span>
                            ))}
                        </div>
                    ...
                    {/* Alerts & Audio */}
                    <section>
                         <h3 className="text-xs font-bold text-[#FFC038]/50 uppercase border-b border-[#FFC038]/20 pb-2 mb-4">Alerts & Audio Panel</h3>
                         ...
                         <div className="space-y-3">
                             ...
                             {/* Voice Style (Drill Sergeant Toggle) */}
                             <div className="p-3 bg-[#140a00] rounded border border-[#FFC038]/10">
                                 <div className="flex justify-between items-center mb-2">
                                     <label className="text-xs text-[#FFC038]/70 block">Voice Alert Style</label>
                                     <span className="text-[10px] text-[#FFC038]/50">Drill Sergeant Mode: {settings.alerts.voiceStyle === 'drill' ? 'ACTIVE' : 'OFF'}</span>
                                 </div>
                             ...
                             <div className="p-3 bg-[#140a00] rounded border border-[#FFC038]/10 flex items-center justify-between">
                                 <div>
                                    <label className="text-xs text-[#FFC038]/70 block mb-1">Tone Profile (SFX)</label>
                                    <select 
                                        value={settings.alerts.toneType}
                                        onChange={(e) => updateSettings({ alerts: { ...settings.alerts, toneType: e.target.value as any } })}
                                        className="bg-black text-[#FFC038] text-xs border border-[#FFC038]/30 rounded p-1 outline-none w-40"
                                    >
                                 ...
                                 <Button onClick={previewTone} variant="secondary" className="text-xs h-8">
                                     <Play className="w-3 h-3" /> Preview SFX
                                 </Button>
                             </div>
                         </div>
                    </section>
